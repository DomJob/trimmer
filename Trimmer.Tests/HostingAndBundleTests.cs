...
public class NuGetPackageResolverTests
{
    ...
    {
        var a = NuGetPackageResolver.ComputeCacheKey([new("A", "1.0"), new("B", "2.0")]);
        var b = NuGetPackageResolver.ComputeCacheKey([new("B", "2.0"), new("A", "1.0")]);
    ...
    {
        var a = NuGetPackageResolver.ComputeCacheKey([new("A", "1.0")]);
        var b = NuGetPackageResolver.ComputeCacheKey([new("A", "2.0")]);
    ...
    {
        var resolver = new NuGetPackageResolver();
        var xml = resolver.BuildProbeProjectXml([new("Humanizer", "2.14.1"), new("Dapper", null)]);
